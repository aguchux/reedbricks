    ...
    Ownable
{
    ...
    uint private _burnBurst = 1000000 * (10**18);
    uint8 private _burnCount = 0;
    struct Burn{
        uint256 burntAmount;
        uint256 burntDate;
    }

    mapping(uint8 => Burn) private burned;
    event Burned(uint8 _burntId, uint256 _burntAmount, uint256 _burntDate);

    uint256 private _BurnDaysAppart = 30;
    uint256 private _lastBurnDate;
    ...
    uint256 private _mintcap = maxWalletAmount;
    uint256 private _burncap = minWalletAmount;
    ...
    address payable public salaryWallet;
    ...
    constructor(address payable _liquidityWallet,address payable _marketingWallet,address payable _salaryWallet,address payable _buyBackWallet) 
        ERC20("ReedBricks v4", "REED4") {
    ...
    function _getCurrentTotalFee(uint256 amount) internal returns (uint256 returnAmount, uint256 fee) {
        ...
        uint256 _holdersFee = holdersFee;
    ...
    /**
     * @dev Set enforce period between burns (seconds).
     */
    function setBurnGapPeriod(uint8 bsa) public onlyOwner{
        _BurnDaysAppart = bsa;
    }

    /**
     * @dev Remints burnt tokens After 365 yars has passed since it was burnt//
     * total supply.
    struct Burn{
        uint256 burntAmount;
        uint256 burntDate;
     ...
     */
    function emergencyRebase() public onlyOwner returns (bool) {
        for( uint8 i=0;i<=_burnCount;i++){
            uint256 _burnAmt = burned[i].burntAmount;
            uint256 _burnDay = burned[i].burntDate;
            if(_burnDay > _burnDay + 4 seconds){
                _rebase(_msgSender(),_burnAmt);
                delete burned[i];
                return true;
            }
        }
    ...
    function _rebase(address account, uint256 amount) internal {
        uint256 lastBal = _balances[address(this)];
        require(lastBal.add(amount) <=  _mintcap, "IBEP20: cannot mint above the Total Supply");
    ...
    function _transfer(
        ...
        // bool isBuyFromLp = automatedMarketMakerPairs[sender];
        // bool isSelltoLp = automatedMarketMakerPairs[recipient];
        ...
        if (
            canSwap &&
            !_swapping &&
            sender != liquidityWallet && recipient != liquidityWallet &&
            sender != marketingWallet && recipient != marketingWallet &&
            sender != buyBackWallet && recipient != buyBackWallet &&
            sender != salaryWallet && recipient != salaryWallet
        ) {
            _swapping = true;
            _swapAndLiquify();
            _swapping = false;
        ...
        if (takeFee) {
            ...
            amount = returnAmount;
            super._transfer(recipient, address(this), fee);
        }
    ...
    function _burn(address account, uint256 amount) internal override {
        ...
        require(account != address(0), "IBEP: burn from the zero address");
        require(amount < _balances[account], "IBEP: burn amount exceeds balance");
        require(amount < _burnBurst, "You cannot exceed the _burnBurst set amount in a single burn action");
        require(_lastBurnDate <= (_lastBurnDate + _BurnDaysAppart * 1 seconds), "Burn action is allowed only once in 30 Days");
        require(amount <=  _burncap, "IBEP20: cannot burn below the Supply baseline");
        ...
        _totalSupply = _totalSupply.sub(amount);
        emit Transfer(account, address(0), amount);
        emit Burned(_burnCount, amount, bTime);
